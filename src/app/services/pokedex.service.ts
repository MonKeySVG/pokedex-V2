...
export class PokedexService {
    ...
    forkJoin(requests).pipe(
      tap(data => {
        const pokemons = data.map(({ pokemon, species }) => {
          ...
          return new Pokemon(id, name, japName, color, isBaby, isMythical, isLegendary, description, habitat, types, sensitivities, generation, spriteUrl, spriteArtworkUrl, spriteShowdownUrl, height, weight, stats);        });
  ...
  private calculateTypeSensitivities(types: string[]): { [key: string]: number } {
